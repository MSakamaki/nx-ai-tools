...
/**
 * Read-only report viewer: serves `GET /api/summary`, `GET /api/sessions`, `GET /api/health`, and
 * the static report-ui bundle from `dist/report-ui`. Never records a metrics event or mutates
 * anything on disk. Returns the listening server (e.g. so callers/tests can close it).
 */
export async function createReportServer(options: ReportServerOptions = {}): Promise<Server> {
