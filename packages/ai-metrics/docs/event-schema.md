...
| `raw_event` | 上記のいずれにも正規化できなかったイベント（例: Claude Codeの未知のhookイベント、Copilotの未知イベント）を、生payloadのまま保存するために使う |
...
| `providerEventName` | `string \| null` | providerが報告する元のイベント名（例: Claude Codeのhook名）。取得不能時は `null` |
| `modelRaw` | `string \| null` | provider報告の生のモデル識別子。取得不能時は `null` |
| `status` | `'success' \| 'error' \| 'cancelled' \| null` | 処理結果。取得不能時は `null` |
| `normalizationStatus` | `'normalized' \| 'partial' \| 'raw_only' \| 'failed'` | 正規化がどこまで成功したか |
| `sourceAvailability` | `SourceAvailability` | 各フィールドが実際に観測できたかどうか |
| `git` | `GitContext` | git関連情報 |
| `agent` | `AgentInfo` | agent使用状況 |
| `skill` | `SkillInfo` | skill使用状況 |
| `rawEvent` | `unknown` | 正規化前のprovider生payload（デバッグ・再処理用） |
| `rawEventTruncated` | `boolean` | `rawEvent` が保存時に切り詰められたかどうか |
...
| `action_started` | `actionId`（必須） |
| `action_completed` | `actionId`（必須）, `tokenUsage`, `responseBody: string \| null` |
| `action_failed` | `actionId`（必須）, `tokenUsage`, `responseBody: string \| null` |
| `session_completed` | `tokenUsage`, `responseBody: string \| null` |
| `raw_event` | 追加フィールドなし（共通フィールドのみ、`rawEvent` に生payloadを保持） |
